...
function sumOfCodes(str) {
  if (str) {
    ...
    let sum = 0;
    for (let i = 0; strArray.length > i; i += 1) {
      sum += strArray[i].charCodeAt(0);
    }
    return sum;
  }
  return 0;
...
function countVowels(str) {
  return str.length - str.replace(/[aeiouy]/gi, '').length;
}
...
function isPalindrome(str) {
  const newStr = str.replace(/[^\w\s]|_|\s+/g, '');
  return !!(
    newStr.toUpperCase() === newStr.split('').reverse().join('').toUpperCase()
  );
}
...
function findLongestWord(sentence) {
  return sentence.split(' ').sort(function sorting(a, b) {
...
function reverseWords(str) {
  ...
  const answerArray = [];
  for (let i = 0; strArray.length > i; i += 1) {
    answerArray.push(strArray[i].split('').reverse().join(''));
...
function invertCase(str) {
  ...
  for (let i = 0; strArray.length > i; i += 1) {
    if (strArray[i] === strArray[i].toUpperCase()) {
      answerArray.push(strArray[i].toLowerCase());
    } else answerArray.push(strArray[i].toUpperCase());
  }
  return answerArray.join('');
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; strArray.length > i; i += 1) {
    if (alphabetArray.includes(strArray[i])) {
      const wordIndex = alphabet.indexOf(strArray[i]);
      answerArray.push(decodeArray[wordIndex]);
    } else answerArray.push(strArray[i]);
  }
